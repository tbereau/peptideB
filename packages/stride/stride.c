...
void ProcessStrideOptions(char **List, int ListLength, COMMAND *Cmd)
{
  ...
  for( i=1; i<ListLength; i++ ) {
	...
	Cmd->MolScript = YES;
		  ...
		  Cmd->PBC = YES;
		  Cmd->PBC_size = atof(List[i]+2);		  
		  if ( Cmd->PBC_size == 0.) {
			  printf("Warning: Periodic Boundary Condition box size was set to 0.\n");			  
		  }		  
	  }	 
...
void PrintStrideHelp(COMMAND *Cmd)
{
  ...
  fprintf(stderr,"  -aNumber    Periodic boundary condition of box size Number\n");  
