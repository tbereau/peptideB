...
int FindHydrogenBonds(CHAIN **Chain, int NChain, HBOND **HBond, COMMAND *Cmd)
{
	...
	for( dc=0; dc<NDnr; dc++ ) {
		...
		for( ac=0; ac<NAcc; ac++ ) {
			...
			if (Cmd->PBC == YES) {
				// Try to move the atoms involved in HBonding so that the minimal distance applies
				MinDist(Dnr[dc]->Chain->Rsd[Dnr[dc]->D_Res]->Coord[Dnr[dc]->D_At],
						Acc[ac]->Chain->Rsd[Acc[ac]->A_Res]->Coord[Acc[ac]->A_At],Cmd->PBC_size,shift);
				// Now apply the transformation to the donor particles H, D_At
				for( j=0; j<3; ++j){
					Dnr[dc]->Chain->Rsd[Dnr[dc]->D_Res]->Coord[Dnr[dc]->H][j]      += shift[j]*Cmd->PBC_size;				
					Dnr[dc]->Chain->Rsd[Dnr[dc]->D_Res]->Coord[Dnr[dc]->D_At][j]   += shift[j]*Cmd->PBC_size;				
					Dnr[dc]->Chain->Rsd[Dnr[dc]->DD_Res]->Coord[Dnr[dc]->DD_At][j] += shift[j]*Cmd->PBC_size;
				}
			}
