...
**         *Coord2   Coordinates of the second point                    **
**                                                                      **
** RETURNS:          what is the minimal distance between two points    **
**                                                                      **
*************************************************************************/
void MinDist(float *Coord1, float *Coord2, float boxsize, int *shift)
{
	register int i;
	float Dist=0;	
	
	for( i=0; i<3; ++i ) {
		if (fabs(Coord1[i]-Coord2[i]+boxsize) < fabs(Coord1[i]-Coord2[i]))
			shift[i] = +1;	
		else if (fabs(Coord1[i]-Coord2[i]-boxsize) < fabs(Coord1[i]-Coord2[i]))
			shift[i] = -1;
		else
			shift[i] =  0;		
	}
