...
namespace eval peptideb {
	...
	# default, but can be set in the parameter file.
	...
	proc read_AA_sequence { file } {
		...
		for { set k 1 } { $k < [llength [lindex $peptideb::amino_acids $j]] } { incr k  } {
			...
			if {[llength [lindex [lindex $peptideb::amino_acids $j ] [expr $k-1]]] == 4 } {
			    # set the omega angle
			    set omega_k [::peptideb::utils::deg2rad [lindex [lindex [lindex $peptideb::amino_acids $j ] [expr $k - 1]] 3]]
			}
		    } else {
		    ...
		    add_N_atom $psi_k
		    # The next Calpha atom can either assume an angle of 180 degrees or read off a value.
		    add_Ca_atom $omega_k
